...
TEST_CASE("ArrayObject indexer access", "[array]") {
    ...
    SECTION("Valid index") {
        REQUIRE(arr.InvokeGetIndexer(Value(0)).ToInteger() == 10);
        REQUIRE(arr.InvokeGetIndexer(Value(2)).ToInteger() == 30);
    }

    SECTION("Invalid index throws") {
        REQUIRE_THROWS_AS(arr.InvokeGetIndexer(Value(-1)), RuntimeError);
        REQUIRE_THROWS_AS(arr.InvokeGetIndexer(Value(3)), RuntimeError);
    }
}
...
TEST_CASE("ArrayObject index assignment", "[array]") {
    ...
    arr.InvokeSetIndexer(Value(1), Value(42), TokenType::Assign);
    REQUIRE(arr.Items()[1].ToInteger() == 42);

    REQUIRE_THROWS_AS(arr.InvokeSetIndexer(Value(5), Value(99), TokenType::Assign), RuntimeError);
}
